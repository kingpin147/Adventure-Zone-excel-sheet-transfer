...
export const testGetLast10DaysBookings = webMethod(Permissions.Admin, async () => {
    try {
        ...
        return bookings.map(rootBooking => {
...
export const testPreviousDeveloperData = webMethod(Permissions.Admin, async () => {
    try {
        const elevatedQuery = auth.elevate(extendedBookings.queryExtendedBookings);
        
        // Query the last 10 days to get some sample data
        const now = new Date();
        const startWindow = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 10, 0, 0, 0);
        const endWindow = new Date(now.getTime());
        ...
        const q = {
            ...
            cursorPaging: { limit: 10 } // just grab a few
        };

        const results = await elevatedQuery(q);
        const bookings = results.extendedBookings || [];

        // Map EXACTLY how the previous developer did (without b.booking wrapping or fallback paths)
        return bookings.map(b => {
             const res = b.formInfo?.extendedFormResponses || {};
             const serviceName = b.bookedService?.name || "";
             return {
                 extractedId: b._id, // This will likely be undefined
                 extractedStartDate: b.selectedSession?.start?.timestamp || "",
                 extractedContactEmail: b.contactDetails?.email || "",
                 extractedServiceName: serviceName,
                 internalNotes: b.notes || b.internalNotes || "",
                 booleanValP: res["c_5951def1_1464_448e_97f3_d748c65c4c96"] || false,
                 RAW_PAYLOAD: b // Returning the raw payload to let you inspect what they were querying against
             };
        });
